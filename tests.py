...
class TestCase:
    ...
    def test_get_all_persons(self, test_client, init_database):
        res = test_client.get('/persons')
        assert res.status_code == 200
        assert len(res.get_json()) == len(Person.query.all())

    def test_get_person_by_id(self, test_client, init_database):
        res = test_client.get('/persons/1')
        assert res.status_code == 200
        assert Person.query.get(1).firstname == 'TestFirstName'

    def test_get_person_not_exist(self, test_client, init_database):
        res = test_client.get('/persons/3000')
        assert res.status_code == 404

    def test_post_new_person(self, test_client, init_database):
        data = {
            "firstname": "UnitName",
            "surname": "UnitSur",
            "birth": "1998-04-17",
    ...
    def test_post_new_invalid_person(self, test_client, init_database):
        data = {
            "firstname": "UnitName",
            "birth": "1998-04-17",
            "job": "UnitJob",
    ...
    def test_patch_person(self, test_client, init_database):
        data = {
            "firstname": "UnitName2",
            "job": "UnitJob2"
        }
    ...
    def test_delete_person(self, test_client, init_database):
